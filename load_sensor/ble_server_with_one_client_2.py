...
def conversion(value, sensor_n, file_name, dt=None):
    ...
    timestamp = time.time()  # float seconds since epoch, like the other sensor code
    data = f"{timestamp:.8f}, dt: {dt:.3f}s, pressure: {pressure}, percentage: {percentage}, temperature: {temperature}, press_counts: {press_counts}, temp_counts: {temp_counts}"
    datafile = f"{file_name} pressure_sensor {sensor_n}.txt"

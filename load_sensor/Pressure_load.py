...
def pressure_loop(address_pico, handle_number, name_cluster):
    last_time = time.time()
    while True:
        peripheric_pico(address_pico, handle_number, name_cluster, last_time)
        time.sleep(0.1)
...
# === MAIN ===
def main():
    # Start pressure logging in separate thread
    ...
    # Start load sensor asyncio loop in main thread
